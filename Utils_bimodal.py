...
def gp_train_bimodal(N_data, bounds):
    """
    ...
    outputs = f_bimodal(inputs)
    
    noise_cov_train = 0.01 * np.array([[1 / np.sqrt(0.2), 0.0],
                                    [0.0, 1 / np.sqrt(0.75)]])
...
def create_observations(N):
    ...
    """
    D = 2
    measures = np.array([0, 2])
    measures = np.tile(measures, (N, 1))
    
    cov_obs = 5 * np.array([[1 / np.sqrt(0.2), 0.0],
                            [0.0, 1 / np.sqrt(0.75)]])
...
def sample_mcmc(n_mcmc, gp, observations, cov_obs_tot, bounds_mcmc, n_tune=1000, input_names=[r'$x_1$', r'$x_2$']):
    ...
    """
    sampler = NUTS_Bimodal(gp, observations, cov_obs_tot, bounds_mcmc, input_names=input_names)
...
def sequential_design_sur(sampler_sur, gp_sur, observations, cov_obs_tot, iterations,
    ...
    Parameters
    ----------
    sampler_sur : NUTS_Bimodal object
...
def kl(sampler, sampler_inf, thinning=100, burnin=500, kde=None, kde_inf=None):
    ...
    sampler : NUTS_Bimodal object
        MCMC sampler with target distribution p_n.
    sampler_inf : NUTS_Bimodal object
